...
import { ManagerId } from "src/transactions/shared/domain/ids/ManagerId";
import { PhoneNumber } from "src/transactions/shared/domain/ids/PhoneNumber";
import { TokenPair } from "src/transactions/shared/domain/TokenPair";
import { Uuid } from "src/transactions/shared/domain/Uuid";
import { StudentValueId } from "src/transactions/shared/domain/ids/StudentValueId";
...
export class JwtServiceNest implements JwtService{
    ...
    private async createRefreshToken(refreshToken: AccessToken): Promise<AccessToken>{
        const { role,sub } = this.jwtNestService.decode(refreshToken.getValue()) as JwtPayload;
    ...
    private async createForStudent(studentId: StudentValueId): Promise<AccessToken>{
        const payload = {
            role: Role.STUDENT,
    ...
    async createForSettingPassword(accountId: StudentValueId): Promise<AccessToken> {
        const payload = {
    ...
    async verify(jwt: AccessToken): Promise<boolean> {
        ...
        .catch((error) => {
            ...
            const description = 
            code === 'JWT_EXPIRED' ? `Expired at ${error.expiredAt.toISOString()}` : '';
    ...
    async createTokenPairForStudent(studentId: StudentValueId): Promise<TokenPair> {
